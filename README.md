...
flowchart TB
    ...
    subgraph Data Engineering Pipeline & Automation
        SCHED(("APScheduler\nProducer-Consumer")) -->|"Every 90s"| Producer
        
        Producer["Batch Scraper\n(Trafilatura + BS4)"] -->|"Raw Articles"| Queue[("In-Memory Queue")]
        Queue -->|"Every 5s (Ticker)"| Consumer["Consumer Data Engine\nRegex + Pandas + Timestamping"]
        Consumer --> Structuring["Metadata Extraction"]
    end
...
```mermaid
flowchart TD
    Start["Producer Runs (90s interval)"] --> Scrape["Scrape Batches from Rotating Sources"]
    Scrape --> Queue["Push Raw Articles to Memory Queue"]
    
    Queue --> Ticker["Consumer Ticks (5s interval)"]
    Ticker --> Pop["Pop 1 Article from Queue"]
    Pop --> Clean["Sanitize, Normalize, & Extract HTML"]
    Clean --> Timestamp["Stamp `generated_at`"]
    
    Timestamp --> DB["Insert to PostgreSQL"]
    DB --> EnforceCap{"Total > 100?"}
    EnforceCap -- Yes --> Prune["Delete Oldest Article"]
    Prune --> Finish["Wait for next 5s Tick"]
    EnforceCap -- No --> Finish
```
...
A Producer-Consumer automation system runs asynchronously via `APScheduler`:
- **Producer:** A background job running every 90 seconds scrapes a complete page of articles from one rotating source and buffers them into an in-memory queue.
- **Consumer:** A continuous ticker fires every **5 seconds**, popping exactly one article from the queue, cleaning it, and inserting it into the database to create a simulated "real-time" continuous data feed.
- **Maintenance/Pruning:** The pipeline enforces a strict **100-article cap**, automatically pruning the oldest data at every insertion. 
- **Real-Time UI Tracker:** A live TV channel-style marquee ticker on the dashboard visualizes articles the instant they are generated by this consumer.
